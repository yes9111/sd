...
import sd.dbbrowser.controller : DBBrowserController;
import sd.dbbrowser.model : DBBrowserModel;
import sd.type.table;
import sd.type.database : Database;
import sd.type.matrix;
import sd.matrixviewer.viewer : MatrixViewer;
...
final class DBBrowser : MainWindow
{
	...
	private DBBrowserModel model;

	/// Construct a new DB browser window
	this()
	{
		super("Explore SQL");
		model = new DBBrowserModel();
		controller = new DBBrowserController(model);
...
private:
	...
	void addDB(in Database database)
	{
		...
		list.addOnRowActivated((path, column, view) {
			...
			{
				errorf("Failed to create table editor for table %s: Exception %s", tableName, e,);
			}
		});
